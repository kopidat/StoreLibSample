...
public class PartnerStoreSampleActivity extends AppCompatActivity {

    private static final String TAG = PartnerStoreSampleActivity.class.getSimpleName();
    ...
    private void auth() {
        ...
        SKIMP_Store_P_Lib.getInstance().auth(this, new LibHelper.OnResultListener() {
            @Override
    ...
    // 로그인
    private void login() {
        SKIMP_Store_P_Lib.getInstance().login(this, "my_app_scheme");
        finishAffinity();
    } // private void login() {

    // 앱 업데이트
    private void update() {
        SKIMP_Store_P_Lib.getInstance().update(this, "my_app_scheme");
        finishAffinity();
    } // private void update() {

    // 스토어앱 설치페이지 이동
    private void goAppStoreInstallPage() {
        SKIMP_Store_P_Lib.getInstance().goAppStoreInstallPage(this);
        finishAffinity();
    } // private void goAppStoreInstallPage() {

    // 스토어 로그인 사용자 정보 가져오기
    private void getUserInfo() {
        LibHelper.LoginUserInfo userInfo = SKIMP_Store_P_Lib.getInstance().getUserInfo(this);
